...
| [Polynomial discriminant](benchmarks/8-polydisc/README.md) | `polydisc` | [Submission.lean](benchmarks/8-polydisc/official/Submission.lean) | Available | Wall time, Callgrind, PMU |
...
`diagnostic` prepares exact-output targets for selected inputs in all eight problems and measures kernel target replay. It keeps individual samples, export hashes, step logs, and per-case failures. It checks the selected output equalities and exported axioms; it does not verify the universal correctness theorem.

Matrix-permanent inputs pack a dimension and seed as `(dimension << 32) | seed`. Its default diagnostic uses all 15 cases from the pinned evaluator's unseeded local plan and an 8192 MiB memory watchdog. Rule 110 and SHA-256 pack `(steps << 32) | seed` and each use six local public cases: 2, 4 and 8 steps for Rule 110; 4, 32 and 512 for SHA-256. Polynomial discriminant uses six uniformly sampled inputs from the same local plan, with two cases in each of three coefficient-width bands. Fibonacci, partition, Mertens and prime counting use six documented endpoints.

All except permanent default to a 4096 MiB watchdog. Override these defaults with `--inputs` and `--memory-mb`; see the [permanent](benchmarks/5-permanent/README.md#run-diagnostics), [Rule 110](benchmarks/6-ca-rule110/README.md#run-diagnostics), [SHA-256](benchmarks/7-sha256/README.md#run-diagnostics), and [polynomial discriminant](benchmarks/8-polydisc/README.md#run-diagnostics) guides.
