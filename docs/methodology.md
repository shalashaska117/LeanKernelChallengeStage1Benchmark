...
`python3 benchmark.py diagnostic` supports all eight problems. For `fib`, `partition`, `mertens` and `primecount` it uses the six public range endpoints listed in each problem's `cases.json`. For `permanent` it uses the 15 packed inputs from the pinned evaluator's unseeded local plan, with five cases each at dimensions 6, 12 and 16. Rule 110 and SHA-256 each use six packed inputs from that plan, with two seeds per step count: 2, 4 and 8 for Rule 110; 4, 32 and 512 for SHA-256. Polynomial discriminant uses the six uniform-integer samples from that plan, with two cases per D1, D3 and D5 group. The `--inputs` option selects an explicit custom set.
...
SHA-256 answers use Python's `hashlib.sha256`. The seed expands once into eight successive 32-bit LCG states, encoded as 32 bytes in big-endian order. Each step hashes the previous digest's binary bytes, preserving leading zero bytes. Zero steps returns the expanded seed digest. Reports retain decoded steps and seed, with the exact natural-number output stored as a decimal string.

Polynomial discriminant answers use a separate Python implementation of the coefficient generator. It builds the full 47-by-47 Sylvester matrix of the degree-24 polynomial and its mathematical derivative, then evaluates its determinant with exact Bareiss elimination. This oracle uses only the Python standard library. The complete `Nat` input selects a coefficient-width band and initializes the generator. Reports retain degree, maximum coefficient width, actual coefficient bit sizes, and a group and case index for inputs in the public local plan. Signed outputs are decimal strings, including values beyond Python's default integer-string length limit.
